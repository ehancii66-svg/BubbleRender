...
\title[肥皂泡实时渲染]{肥皂泡实时折射与薄膜虹彩渲染系统}
\subtitle{屏幕空间折射、薄膜干涉与表面动态}
...
\begin{frame}{项目背景}
  \begin{columns}[T]
    \begin{column}{0.56\linewidth}
      ...
      \vspace{0.6em}
      \begin{itemize}
        \item \textbf{题目方向：} 透明物体、折射、薄膜干涉与动态表面渲染。
        \item \textbf{交付：} Windows 桌面端源码、实验报告、论文汇报 PPT 与 demo 视频。
      \end{itemize}
    \end{column}
    \begin{column}{0.40\linewidth}
      ...
      \end{block}
      \begin{block}{实现目标}
        在普通 Windows 图形环境中保持实时运行，并让主要物理/光学现象可以被观察和解释。
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\linewidth}
      \begin{itemize}
        ...
        \item \textbf{实时交互：} 需要支持参数调节、鼠标触摸和实时演示。
      \end{itemize}
      \vspace{0.4em}
...
\begin{frame}{系统组成}
  \begin{table}
    ...
    \begin{tabular}{p{0.18\linewidth}p{0.32\linewidth}p{0.40\linewidth}}
      ...
      模块 & 主要内容 & 作用 \\
      \midrule
      渲染管线 & FBO、天空盒、绘制顺序 & 组织三阶段折射与最终合成 \\
      泡泡材质 & 折射 shader、薄膜虹彩、Fresnel & 形成透明轮廓和干涉色 \\
      动态模拟 & DBSTT / vortex sheet & 驱动主泡泡轻微形变 \\
      交互控制 & 鼠标、键盘参数 & 支持展示和局部扰动 \\
      场景展示 & 18 个副泡泡、风场漂移 & 增强空间层次和演示观感 \\
      \bottomrule
    \end{tabular}
  \end{table}
  \vspace{0.2em}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.50\linewidth}
      ...
      \begin{enumerate}
        ...
        \item \textbf{背面阶段：} 只绘制泡泡背面，采样背景纹理，得到第一次界面折射。
        \item \textbf{正面阶段：} 绘制泡泡正面，采样背面 FBO，并叠加薄膜虹彩、Fresnel 和环境反射。
      \end{enumerate}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{折射偏移与边缘处理}
  ...
  \end{columns}
  \vspace{0.4em}
  为减少副泡泡采样黑边或变黑，FBO 使用 1.3 倍 overscan，并在 shader 中修正窗口坐标与 FBO 坐标的映射。
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\linewidth}
      ...
      \begin{itemize}
        ...
        \item \(M_{\mathrm{touch}}\)：右键触摸点附近的局部膜厚增强。
...
\begin{frame}{DBSTT / Vortex Sheet 表面动态（1/2）}
  \small
  \begin{columns}[T]
    \begin{column}{0.50\linewidth}
      \textbf{物理思想}
      \begin{itemize}
        \item 将泡泡表面视为 \textbf{vortex sheet}，用表面标量环量 \(\Gamma\) 编码涡量状态。
        \item 表面张力通过曲率差驱动 \(\Gamma\) 变化，再由 Biot-Savart 积分将涡量反推为速度场，推动表面运动。
        \item 区别于 sin/cos 参数动画——泡泡的晃动来源于真实的涡量-表面张力能量交换。
      \end{itemize}
      \vspace{0.3em}
      \textbf{核心公式链（Da et al.\ 2015）}
      \begin{enumerate}
        \item 曲率 \(\rightarrow\) 环量：\(\Delta\Gamma^v \propto \frac{\sigma}{\rho}(H_1^v-H_2^v)\)
        \item 环量 \(\rightarrow\) 涡片强度：\(\boldsymbol{\gamma}=\mathbf{n}\times\nabla_f\Gamma\)
        \item 涡片 \(\rightarrow\) 速度场：\(\mathbf{u}(\mathbf{x})=\frac{1}{4\pi}\int\frac{\boldsymbol{\gamma}\times\mathbf{r}}{(|\mathbf{r}|^2+\alpha^2)^{3/2}}\,\mathrm{d}A\)
        \item 速度 \(\rightarrow\) 新位置：前向 Euler 积分
      \end{enumerate}
    \end{column}
    \begin{column}{0.47\linewidth}
      \begin{block}{我们的实现要点}
        \begin{itemize}
          \item 离散曲率采用 \textbf{cotan 公式}（拉普拉斯-贝尔特拉米算子），避免原论文二面角法的符号判定问题。
          \item 发现论文原始 d\(\Gamma\) 符号对单闭合泡泡产生破坏性反馈，经过系统性符号追踪后反号修正。
          \item 数值稳定性：位移钳制（\(\le 0.1\times\)边长）、质心校正、拉普拉斯环量扩散。
          \item 每帧更新：法线/面积 \(\rightarrow\) 曲率 \(\rightarrow\) 环量 \(\rightarrow\) \(\boldsymbol{\gamma}\) \(\rightarrow\) Biot-Savart \(\rightarrow\) 推进顶点 \(\rightarrow\) 上传 VBO。
        \end{itemize}
      \end{block}
      \begin{block}{参数控制}
        \scriptsize
        表面张力（键 3/4）、环量扩散（5/6）、子步数（7/8）、d\(\Gamma\) 符号翻转（9）、暂停/重置（Z/X）
      \end{block}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{DBSTT / Vortex Sheet 表面动态（2/2）}
  \begin{columns}[T]
    \begin{column}{0.52\linewidth}
      \textbf{效果特点}
      \begin{itemize}
        \item 泡泡从拉伸椭球初始状态开始，表面张力试图恢复球形，涡量惯性导致过冲，形成往复振荡。
        \item 环量扩散提供阻尼，振荡幅度随时间衰减，最终趋近球形平衡。
        \item 形变幅度可通过表面张力系数实时调节（1--50），演示时可展示从轻微晃动到剧烈形变的连续变化。
      \end{itemize}
      \vspace{0.4em}
      \textbf{适用性与局限}
      \begin{itemize}
        \item 项目中仅用于单个闭合主泡泡，不涉及多泡泡合并/分裂（完整 DBSTT 泡沫拓扑变化）。
        \item 网格固定为 icosphere 细分等级 3（642 顶点），保证 Biot-Savart O(VF) 计算量可控。
      \end{itemize}
    \end{column}
    \begin{column}{0.44\linewidth}
      \centering
      \fig{1.0}{bubble_wobble1.png}
      \vspace{0.3em}
      \scriptsize 形变过程中法线和轮廓的变化
      \vspace{0.8em}

      \centering
      \fig{1.0}{bubble_wobble2.png}
      \vspace{0.3em}
      \scriptsize 收敛至球形后的稳定状态
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{右键触摸交互}
  \begin{columns}[T]
    \begin{column}{0.32\linewidth}
      \centering
      \fig{1.0}{fig_touch_none.png}\\[0.2em]
      \small 无触摸
    \end{column}
    \begin{column}{0.32\linewidth}
      \centering
      \fig{1.0}{fig_touch_press_center.png}\\[0.2em]
      \small 按下触摸
    \end{column}
    \begin{column}{0.32\linewidth}
      \centering
      \fig{1.0}{fig_touch_drag_ripple.png}\\[0.2em]
      \small 拖动波纹
    \end{column}
  \end{columns}
  \vspace{0.5em}
  \begin{itemize}
    \item 左键拖动旋转视角，滚轮缩放；右键用于模拟接触泡泡表面的局部扰动。
    \item 触摸不会作为全局折射参数，而是在接触点附近增强膜厚、折射和色散。
    \item 副泡泡禁用触摸项，只作为场景展示元素。
  \end{itemize}
\end{frame}
...
\begin{frame}{结果与局限}
  \begin{columns}[T]
    \begin{column}{0.48\linewidth}
      \begin{block}{已实现效果}
        \begin{itemize}
          \item 三阶段 FBO 双界面折射近似
          \item 三种薄膜虹彩模式实时切换
          \item Fresnel 边缘、环境反射和动态膜厚
          \item 右键触摸波纹与局部光学响应
          \item 主泡泡 DBSTT 风格形变和 18 个漂移副泡泡
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\linewidth}
...
\begin{frame}{总结}
  \begin{itemize}
    \item 项目构建了一个可实时运行的 Windows OpenGL 肥皂泡渲染系统。
    \item 渲染上使用 background FBO、back-face FBO 和 final pass 近似双界面折射。
    \item 光学上实现 Kim2012、Spectral LUT 和 Belcour Airy 三种薄膜虹彩模式。
    \item 动态上结合程序化膜厚、右键触摸波纹、DBSTT 主泡泡形变和多泡泡风吹漂移。
