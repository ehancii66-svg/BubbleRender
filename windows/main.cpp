...
static void FramebufferSizeCallback(GLFWwindow* window, int width, int height)
{
    ...
    if (g_BackgroundFBO)  glDeleteFramebuffers(1, &g_BackgroundFBO);
    if (g_BackgroundTexture) glDeleteTextures(1, &g_BackgroundTexture);
    if (g_BackFaceFBO)    glDeleteFramebuffers(1, &g_BackFaceFBO);
    if (g_BackFaceTexture)  glDeleteTextures(1, &g_BackFaceTexture);
    if (g_DepthRB)        glDeleteRenderbuffers(1, &g_DepthRB);
...
static void Cleanup()
{
    ...
    if (g_CubemapTexture)     glDeleteTextures(1, &g_CubemapTexture);
    if (g_BackgroundFBO)      glDeleteFramebuffers(1, &g_BackgroundFBO);
    if (g_BackgroundTexture)  glDeleteTextures(1, &g_BackgroundTexture);
    if (g_BackFaceFBO)        glDeleteFramebuffers(1, &g_BackFaceFBO);
    if (g_BackFaceTexture)    glDeleteTextures(1, &g_BackFaceTexture);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(g_WindowWidth, g_WindowHeight,
                                          "BubbleRender - Refraction Demo", nullptr, nullptr);
    if (!window) {
