...
\reportfigure{placeholder_multi_bubble_contact.png}{0.72}{0.3\textheight}{多泡泡接触场景：三个真实模拟泡泡形成接触网络，右侧为融合结果，左侧为独立泡泡}

\subsection{共享膜与接触圆}

\reportfigure{placeholder_contact_circle.png}{0.55}{0.26\textheight}{接触圆与共享膜：泡泡间形成圆形共享膜，球壳在接触平面处被裁剪}
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.30\linewidth}
    \centering
    \fbox{\begin{minipage}[c][0.22\textheight][c]{0.9\linewidth}
      \centering
      \small 分离
    \end{minipage}}\\[0.3em]
    \small (a) 分离
  ...
  \begin{minipage}[t]{0.30\linewidth}
    ...
    \small (c) 融合
  \end{minipage}
  \caption{三种接触结果对比示意图（请替换为实际截图）}
  \label{fig:three_outcomes}
\end{figure}

\subsection{弯曲共享膜}

\reportfigure{placeholder_curved_film_comparison.png}{0.65}{0.24\textheight}{不同尺寸比下的弯曲共享膜：左为等尺寸（平面膜），中为大泡-小泡（膜向大泡凸出），右为极端尺寸比}
