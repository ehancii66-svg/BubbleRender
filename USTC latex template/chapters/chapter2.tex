...
\begin{enumerate}
  ...
  \item \textbf{主场景阶段}：绘制包含主泡泡的场景到 \codepath{mainSceneFBO}，供展示泡泡采样。
  \item \textbf{最终合成阶段}：将主泡泡与多展示泡泡合成到 \codepath{finalSceneFBO}，再输出到屏幕。
\end{enumerate}
...
该修正对多泡泡展示尤其重要。多展示泡泡在不同 pass 中复用同一套折射与虹彩 shader，如果输入坐标不能正确映射到 FBO 空间，就会出现采样越界或局部发黑。修正后，主泡泡和展示泡泡都沿用统一材质逻辑，并通过模型矩阵、局部几何扰动强度、虹彩模式、输出透明度以及是否响应交互等参数来区分。
...
其中 $\mathbf{T}_{xy}$ 是视图空间折射方向的屏幕分量，$s_{\mathrm{ref}}$ 是用户可调的 \codepath{uRefractionStrength}，$s_{\mathrm{face}}$ 区分正面和背面，$s_{\mathrm{film}}$ 由薄膜厚度和边缘程度共同决定，$r_{\mathrm{px}}$ 是泡泡屏幕半径，$s_{\mathrm{edge}}$ 是边缘增强，$s_{\mathrm{touch}}$ 是局部触摸扰动增强。
...
\reportfigure{a2d012859fa31c28b29bf51500bc310f.jpg}{0.5}{0.26\textheight}{背景小球经过主泡泡后的折射扭曲效果}
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{e8e69a8261df5113fa6c9bc42bf782a3.jpg}\\[0.4em]
    \small Kim2012
  \end{minipage}
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{2ee50ccc70b9857b3fc7fca4dac15b94.jpg}\\[0.4em]
    \small Belcour Airy
  \end{minipage}
  \caption{不同虹彩模式对比}
...
其中 $d_0$ 是基础膜厚，$A_{\mathrm{var}}$ 是膜厚变化幅度，$P(\mathbf{n},t)$ 是由多层噪声和排液项组合的厚度图案，$M_{\mathrm{touch}}$ 是触摸中心的局部增强，$Q_{\mathrm{ripple}}$ 是触摸拖动产生的环形波纹。
