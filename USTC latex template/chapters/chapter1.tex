...
本技术文档将介绍本队最终初赛提交版本的核心技术路线。系统围绕一个主泡泡展开：主泡泡负责展示折射、虹彩、触摸扰动和表面形变；周围的展示泡泡负责增强多目标层次与现场演示观感。实现上，系统被组织为渲染管线、薄膜材质、几何数据、动态模拟和资源加载几个相对独立的模块，并通过 ArkTS 页面层与 Native 渲染层共同完成最终应用。

\reportfigure{df945d3735ced72e359e184a6d30f648.jpg}{0.44}{0.28\textheight}{最终整体效果：主泡泡、背景折射、天空盒环境、虹彩边缘以及多个漂移展示泡泡。}
...
\end{quote}
表 \ref{tab:structure} 列出了和实现相关的各个模块。
